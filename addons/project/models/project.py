...
class Task(models.Model):
    ...
    write_date = fields.Datetime(
        index=True)  # not displayed in the view but it might be useful with base_action_rule module (and it needs to be defined first for that)
    ...
    cost_time = fields.Float(string='时长')
    color = fields.Integer(string='Color Index')
    ...
    def _notification_recipients(self, message, groups):
        ...
        new_group = (
            'group_project_user', lambda partner: bool(partner.user_ids) and any(
                user.has_group('project.group_project_user') for user in partner.user_ids), {
